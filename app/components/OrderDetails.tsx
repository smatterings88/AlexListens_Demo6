...
function prepOrderDetails(orderDetailsData: string): OrderDetailsData {
  try {
    ...
    const orderDetails: OrderDetailsData = {
      items: validItems,
      totalAmount: 0 // We don't need to calculate total amount anymore
    };
...
const OrderDetails: React.FC = () => {
  ...
  return (
    <div className="mt-10">
      <h1 className="text-xl font-bold mb-4">Conversation Details</h1>
      <div className="shadow-md rounded p-4">
        <div className="mb-4">
          <span className="text-gray-400 font-mono mb-2 block">Keywords:</span>
          {orderDetails.items.length > 0 ? (
            orderDetails.items.map((item, index) => formatOrderItem(item, index))
          ) : (
            <span className="text-gray-500 text-base font-mono">Waiting for Keywords</span>
          )}
        </div>
      </div>
    </div>
